		...
		while (len > 0) {
				...
				if (!cur) {
					...
					if (tmux_hdr.type == DATA) {
						...
						if (!cur)
							debug(LOG_DEBUG, "stream_id is %d", stream_id);
						assert(cur != NULL);
						if (len == 0) {
							set_cur_stream(cur);
							break;
						...
						if (len >= stream_len) {
							nr = tmux_read(bev, cur, stream_len);
							assert(nr == stream_len);
							len -= stream_len;
						} else {
							nr = tmux_read(bev, cur, len);
							stream_len -= len;
							assert(nr == len);
							set_cur_stream(cur);
							len -= nr;
			 ...
			 struct tmux_stream *stream)
{
	...
	struct common_conf *c_conf = get_common_config();
	if (c_conf->tcp_mux)
		tmux_write(bout, (uint8_t *)req_msg, len, stream);
	else
			 ...
			 struct tmux_stream *stream)
{
	...
	if (get_main_encoder() == NULL) {
		...
		if (c_conf->tcp_mux) 
			tmux_write(bout, coder->iv, 16, stream);
		else
	...
	if (c_conf->tcp_mux)
		tmux_write(bout, enc_msg, olen, stream);
	else
